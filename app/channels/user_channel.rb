class UserChannel < ApplicationCable::Channel
  ...
  def receive(request)
    case request["type"]
      when @@message_types[:join_request]
        then handle_join_request(request["data"]["guest"], request["data"]["host"])
      when @@message_types[:join_request]
        then handle_view_request(request["data"]["guest"], request["data"]["host"])
    end
  end
  ...
  def handle_join_request(guest, host)
    ActionCable.server.broadcast "user_channel_#{host}", { type: @@message_types[:join_request], data: { guest: guest }}
  end

  def handle_view_request(guest, host)
    ActionCable.server.broadcast "user_channel_#{host}", { type: @@message_types[:join_request], data: { guest: guest }}
  end
end
